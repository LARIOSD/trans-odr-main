...
export class UsuariosService extends BaseService<UsuariosQuerys> {
    ...
    async buscar_usuarios(paramHttp: object) {
        let params = JSON.stringify({ ...paramHttp, estado: DB_ESTADOS.ACTIVO })

        const response = await this.query.buscar_usuarios(params)
